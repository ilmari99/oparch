...
default_intervals = {
    ...
    "rate" : [0.0001, 0.005, 0.01, 0.05, 0.095, 0.16,0.2],
...
def set_default_misc(**kwargs):
    ...
    for item in kwargs.items():
        if item[0] not in default_misc:
            print(f"Item {item[0]} not found. Adding ({item[0]}:{item[1]}) to default values.")
        else:
...
def set_default_intervals(**kwargs):
    ...
    for item in kwargs.items():
        if item[0] not in default_intervals.keys():
            print(f"Interval for {item[0]} not found. Adding ({item[0]}:{item[1]}) to default interval values.")
        else:
