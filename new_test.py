...
def y_function(x) -> np.ndarray:
    ...
    features = len(x[0])
    coeffs = np.random.uniform(low=-1, high=1, size=(1,features))[0]#np.array([(c+1)*0.5 for c in range(features)])
...
plt.figure(2)
(loss_fun, loss) = opt.opt_loss_fun(model, X, y)
model.compile(loss=loss_fun,optimizer=tf.keras.optimizers.RMSprop())
...
model.build(np.shape(X))
model.compile(loss=tf.keras.losses.MeanSquaredError(),optimizer=tf.keras.optimizers.RMSprop(lr,decay=decay))
(model, loss_units) = opt.opt_dense_units(model, 0, X, y,return_model=True)
'''
index = 2
if config==None:
    index = 1
